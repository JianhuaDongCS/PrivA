...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    Pre_BPR_Attr = torch.load(args.weights_path + args.dataset + "pre_train_gender" + '.pkl', map_location=args.device)
    u_f_a_embedding = Pre_BPR_Attr['U']
    i_f_a_embedding = Pre_BPR_Attr['I']
    age_embedding = Pre_BPR_Attr['gender']
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            gender_set = gumbel_softmax(G_MLP_output, 1, args)
            ...
            D_loss = model.Discriminator_loss(users, gender, gender_set)
        ...
        if should_stop == True:
            save_path = 'output/%sGender.result' % (args.dataset)
            ensureDir(save_path)
            f = open(save_path, 'a')

            f.write(
                'Gender:  dataset:%s, attribute_dim:%d, lr:%.8f, weight_decay:%.8f, \n\t%s\n'
        ...
        if Loss_all == cur_best_pre_0 and args.save_flag == 1:
            ...
            torch.save(Ageset, args.ageset_path + args.dataset + "Gender_G" + '.pkl')
            #torch.save(Ageset, args.ageset_path + args.dataset + str(args.lambdal)+"_"+str(args.lambdas) + "Gender_G" + '.pkl')
            #torch.save(Ageset,args.ageset_path + args.dataset + "search_Gender_G" + '.pkl')
            #torch.save(Ageset, args.ageset_path + args.dataset + str(args.c) + "Gender_G" + '.pkl')
            print('save the weights in path: ', args.ageset_path + args.dataset + "Gender_G" + '.pkl')
